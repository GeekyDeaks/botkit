...
function Facebookbot(configuration) {
    ...
    facebook_botkit.middleware.format.use(function(bot, message, platform_message, next) {
        ...
        if (message.persona_id) {
            platform_message.persona_id = message.persona_id;
        }
    ...
    facebook_botkit.defineBot(function(botkit, config) {
        ...
        bot.typingDelay = function(message) {
            ...
            let typingDelayFactor = configuration.typingDelayFactor || 1
            ...
            return new Promise(function(resolve) {
                ...
                if (message.typingDelay) {
                    ...
                    typingLength = Math.min(Math.floor(textLength / (avgCPM / 60)) * 1000, 2000) * typingDelayFactor;
                }
        ...
        bot.replyWithTyping = function(src, resp, cb) {
            ...
            bot.startTyping(src, async function(err) {
                // keep trying even with an error due to EU privacy
                if (err) console.error(err);

                await bot.typingDelay(resp)
                bot.say(msg, cb)
            });
        ...
        bot.reply = function(src, resp, cb) {
            ...
            if (resp.typing || resp.typingDelay || facebook_botkit.config.replyWithTyping) {
    ...
    var personas_api = {
        create: function(persona) {
            return facebook_botkit.api.personas.postAPI(persona);
        },
        get_persona: function(persona_id) {
            return facebook_botkit.api.personas.getAPI(persona_id);
        },
        get_all_personas: function() {
            return facebook_botkit.api.personas.getAPI();
        },
        delete_persona: function(persona_id) {
            return facebook_botkit.api.personas.deleteAPI(persona_id);
        },
        postAPI: function(persona) {
            return new Promise(function(resolve, reject) {
                var uri = 'https://' + api_host + '/me/personas?access_token=' + configuration.access_token;
                ...
                request.post(uri,
                    ...
                    function(err, res, body) {
                        if (err) {
                            facebook_botkit.log('Could not create a persona');
                            reject(err);
                        } else {
                            var results = null;
                            ...
                            try {
                                ...
                                facebook_botkit.log('ERROR in creating persona API call: Could not parse JSON', err, body);
                                reject(err);
                            }

                            if (results) {
                                if (results.error) {
                                    facebook_botkit.log('ERROR in creating a persona API call: ', results.error.message);
                                    reject(results.error);
                                } else {
                                    facebook_botkit.debug('Successfully created a persona ', body);
                                    resolve(results);
                                }
                            }
                        }
        ...
        getAPI: function(persona_id) {
            return new Promise(function(resolve, reject) {

                var endpoint = persona_id ? persona_id : 'me/personas';

                var uri = 'https://' + api_host + '/' + endpoint + '?access_token=' + configuration.access_token;
                ...
                request.get(uri,
                    function(err, res, body) {
                        if (err) {
                            facebook_botkit.log('Could not get the persona ' + persona_id);
                            reject(err);
                        } else {
                            var results = null;
                            try {
                                ...
                                facebook_botkit.log('ERROR in getting persona API call: Could not parse JSON', err, body);
                                reject(err);
                            }

                            if (results) {
                                if (results.error) {
                                    facebook_botkit.log('ERROR in getting persona API call: ', results.error.message);
                                    reject(results.error);
                                } else {
                                    facebook_botkit.debug('Successfully got persona ', body);
                                    resolve(results);
                                }
                            }
                        }
        ...
        deleteAPI: function(persona_id) {
            return new Promise(function(resolve, reject) {
                var uri = 'https://' + api_host + '/' + persona_id + '?access_token=' + configuration.access_token;
                ...
                request.delete(uri,
                    function(err, res, body) {
                        if (err) {
                            facebook_botkit.log('Could not delete persona');
                            reject(err);
                        } else {
                            var results = null;
                            try {
                                ...
                                facebook_botkit.log('ERROR in deleting persona API call: Could not parse JSON', err, body);
                                reject(err);
                            }

                            if (results) {
                                if (results.error) {
                                    facebook_botkit.log('ERROR in deleting persona API call: ', results.error.message);
                                    reject(results.error);
                                } else {
                                    facebook_botkit.debug('Successfully deleted persona ', body);
                                    resolve(results);
                                }
                            }
                        }
